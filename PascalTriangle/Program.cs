...
Triangle tr = new Triangle(rows);
Console.WriteLine("Как выводить треугольник? (0 - цифрами, 1 - только нечетные значения в виде *): ");
int format = Convert.ToInt32(Console.ReadLine());
if (format == 0) tr.Print();
else tr.Print(true);

// Эта программа была написана сразу после лекции, поэтому 
// треугольник Паскаля реализован в виде объекта - это было ДЗ на лекции
